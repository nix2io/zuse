...
def get_group_functions(group_name: str) -> List[str]:
    ...
    return [
        ...
        if os.path.isdir(os.path.join(group_folder, name))
    ]
...
def type_to_py(_type: list) -> str:
    ...
    if node_type == "Union":
        return "{} or {}".format(
            type_to_py(node_args["typeLeft"]), type_to_py(node_args["typeRight"])
        )
...
for group in language_specification["groups"]:
    ...
    for func_name in group_function_names:
        ...
        group_functions.append(
            {
                "name": func_name,
                "description": func_spec[1].get("description", "No Description"),
            }
        )
    ...
    groups.append({"label": group[0].upper() + group[1:], "functions": group_functions})
...
create(
    "cheat_sheet",
    "cheat_sheet.html",
    page_name="Cheat Sheet",
    groups=[
        [group for i, group in enumerate(groups) if i % 2 == 0],
        [group for i, group in enumerate(groups) if i % 2 != 0],
    ],
)
